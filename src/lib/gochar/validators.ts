// ============================================================
// Gochar Engine — Validators
// ============================================================

import { SUPPORTED_LANGUAGES } from "@/lib/horoscope/constants";
import { TRANSIT_PLANET_NAMES } from "@/lib/transit/constants";
import type { GocharInput, GocharValidationResult } from "./types";

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
const TIME_RE = /^\d{2}:\d{2}$/;

export function validateGocharInput(input: GocharInput): GocharValidationResult {
  const errors: GocharValidationResult["errors"] = [];
  ...
  if (input.planets !== undefined) {
    for (const p of input.planets) {
      if (!(TRANSIT_PLANET_NAMES as readonly string[]).includes(p)) {
