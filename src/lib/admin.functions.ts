...
export const listUsers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // 3. Discover user_ids from other active tables if not already in userMap
    const [{ data: kundliUsers }, { data: orderUsers }, { data: roleUsers }, { data: entitlementUsers }] = await Promise.all([
      sb.from("user_kundlis").select("user_id, name, created_at"),
      sb.from("orders").select("user_id, customer_name, customer_email, created_at"),
      sb.from("user_roles").select("user_id, created_at"),
      sb.from("user_entitlements").select("user_id, created_at"),
    ]);

    for (const k of kundliUsers ?? []) {
      if (!k.user_id) continue;
      const existing = userMap.get(k.user_id);
      if (existing) {
        if ((!existing.display_name || existing.display_name === "Sanatan User") && k.name) {
          existing.display_name = k.name;
          existing.full_name = k.name;
        }
      } else {
        userMap.set(k.user_id, {
          id: k.user_id,
          email: "",
          display_name: k.name || `User (${k.user_id.slice(0, 6)})`,
          full_name: k.name || `User (${k.user_id.slice(0, 6)})`,
          avatar_url: null,
          provider: "email",
    ...
    for (const o of orderUsers ?? []) {
      ...
      if (existing) {
        ...
        if ((!existing.display_name || existing.display_name === "Sanatan User") && o.customer_name) {
          existing.display_name = o.customer_name;
        }
      } else {
        userMap.set(o.user_id, {
          id: o.user_id,
          email: o.customer_email || "",
          display_name: o.customer_name || o.customer_email?.split("@")[0] || `User (${o.user_id.slice(0, 6)})`,
          full_name: o.customer_name || `User (${o.user_id.slice(0, 6)})`,
          avatar_url: null,
          provider: "email",
    ...
    for (const r of roleUsers ?? []) {
      if (!r.user_id || userMap.has(r.user_id)) continue;
      userMap.set(r.user_id, {
        id: r.user_id,
        email: "",
        display_name: `Staff (${r.user_id.slice(0, 6)})`,
        full_name: `Staff (${r.user_id.slice(0, 6)})`,
        avatar_url: null,
        provider: "email",
    ...
    for (const e of entitlementUsers ?? []) {
      if (!e.user_id || userMap.has(e.user_id)) continue;
      userMap.set(e.user_id, {
        id: e.user_id,
        email: "",
        display_name: `Member (${e.user_id.slice(0, 6)})`,
        full_name: `Member (${e.user_id.slice(0, 6)})`,
        avatar_url: null,
        provider: "email",
