...
export const Route = createFileRoute("/monthly-horoscope/$sign")({
  loader: ({ params }) => {
  ...
  head: ({ params, loaderData }) => {
    ...
    const path = `/monthly-horoscope/${params.sign}`;
    ...
    const title = `${sign.english} (${sign.sanskrit}) Monthly Horoscope | SanatanTools`;
    const desc = `Today's monthly horoscope for ${sign.english} (${sign.hindi}) — career, money, love, health and lucky number, colour and direction from live Vedic planetary calculations.`;
    return {
      meta: [
        { title },
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <NotFoundRashi to="/monthly-horoscope" />
    </SiteLayout>
  ),
