...
export const Route = createFileRoute("/daily-horoscope/$sign")({
  loader: ({ params }) => {
  ...
  head: ({ params, loaderData }) => {
    ...
    const path = `/daily-horoscope/${params.sign}`;
    ...
    const title = `${sign.english} (${sign.sanskrit}) Daily Horoscope | SanatanTools`;
    const desc = `Today's daily horoscope for ${sign.english} (${sign.hindi}) — career, money, love, health and lucky number, colour and direction from live Vedic planetary calculations.`;
    return {
      meta: [
        { title },
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <NotFoundRashi to="/daily-horoscope" />
    </SiteLayout>
  ),
