...
import { Briefcase, TrendingUp, DollarSign, Sparkles, Save, SwitchCamera, BarChart3, Award } from "lucide-react";
...
export function CareerAnalysisAdmin() {
  const [enabled, setEnabled] = useState(true);
  const [priceInr, setPriceInr] = useState(449);
  ...
  const [aiSystemPrompt, setAiSystemPrompt] = useState(
    "You are a Senior Executive Career & Vedic Astrological Strategist. Explain career outcomes with exact D10 Dashamsa, Jaimini Amatyakaraka, 10th house, and Dasha WHY citations. Never generate generic statements."
  );
  ...
  const handleSaveSettings = () => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      toast.success("Career Analysis Admin Settings saved successfully!");
    }, 600);
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4 border-slate-200 dark:border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight">Career Analysis Report Pro — Flagship Admin Control</h1>
          </div>
          <p className="text-sm text-slate-500 mt-1">Configure flagship pricing, subscription tier access, AI persona, and system status.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4 bg-amber-500/5 border-amber-500/20">
          ...
          <div className="text-xl font-bold text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-2">
            <Badge className={enabled ? "bg-amber-500" : "bg-slate-500"}>{enabled ? "FLAGSHIP ACTIVE" : "DISABLED"}</Badge>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-indigo-500/5 border-indigo-500/20">
          ...
          <div className="text-2xl font-black text-indigo-600 dark:text-indigo-400 mt-1">3,850</div>
          <div className="text-xs text-emerald-600 font-semibold">+41.2% this month</div>
        </Card>
        <Card className="p-4 bg-emerald-500/5 border-emerald-500/20">
          <div className="text-xs font-semibold text-slate-500">Total Revenue (30d)</div>
          <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400 mt-1">₹12,96,400</div>
          <div className="text-xs text-slate-400">Avg PDF length: 40 pages</div>
        </Card>
      </div>
